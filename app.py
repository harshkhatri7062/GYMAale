    ...
    if account_info.ok:
        ...
        if account_info_json['login'] == qur.username:
            ...
            adding = github_user(username=account_info_json['login'], password=account_info_json['id'], email=email,
                                 ...
                                 address=address, age=age, interest=interest, already_gymming=already_gymming,
                                 time=time)
...
def send_reset_email(data):
    ...
    with app.app_context():
        ...
        msg.html = render_template("email_message_for_reset.html", link=link, faq=data['faq_link'], token=token,
                                   _external=True)
        mail.send(msg)
...
def send_confirmation_email(data):
    with app.app_context():
        ...
        msg.html = render_template("email_message.html", faq=data['faq_link'], sec_code=data['sec_code'],
                                   token=data['token'], link=data['link'], _external=True)
        mail.send(msg)
...
def account():
    ...
    return render_template("account.html", value=m, value2=z.email, value3=value3, value4=value4,
                           ...
                           wallet_ammount=wallet_qur.ammount, value9=value9, image_url=image_url)
...
def upload():
    ...
    all_labels = []
    s3_client = boto3.client('s3')
    prefix = 'gym_images/' + xyz.username + '_' + str(
        xyz.id)  # setting the name of folder in which images will be uploaded.
    response = s3_client.list_objects(
        Bucket=bucket_name,
        Prefix=prefix
    )
    ...
    if f1_extension and f2_extension and f3_extension and f4_extension and f5_extension in allowed_extensions:
        ...
        if image_bytes1 and image_bytes2 and image_bytes3 and image_bytes4 and image_bytes5:
            ...
            if (
                    'lingre' in all_labels1 or 'Bikini' in all_labels1 or 'Underwear' in all_labels1 or 'Swimwear' in all_labels1 \
    ...
    if f1_extension and f2_extension and f3_extension and f4_extension and f5_extension in allowed_extensions:
        if file1 and file2 and file3 and file4 and file5:
            ...
            new = trainer_image(ref_id=ref_id, image_1_link=upload_result1['url'], image_2_link=upload_result2['url'],
                                image_3_link=upload_result3['url'], image_4_link=upload_result4['url'],
                                image_5_link=upload_result5['url']
...
def trainer_account():
    ...
    return render_template("trainer_registeration/trainer_account.html", zz=zz, mm=mm, image_1_link=image_1_link,
                           image_2_link=image_2_link, image_3_link=image_3_link, image_4_link=image_4_link,
                           image_5_link=image_5_link, decide=decide, trainer_account_balance=trainer_wallet_qur.ammount)
...
def send_ammount_added_to_wallet_email(data):
    with app.app_context():
        ...
        msg.html = render_template('email_conformation_ammount_added_to_wallet.html', data=data,
                                   ammount=data['startAmmount'], _external=True)
        mail.send(msg)
...
def send_user_email_about_trainer_booking(data):
    with app.app_context():
        ...
        msg.html = render_template("email_user_deduction_and_trainer_details.html", trans=data['trans'], data=data,
                                   _external=True)
        mail.send(msg)
...
def send_trainer_ammount(data):
    with app.app_context():
        ...
        msg.html = render_template("email_send_trainer_ammount_and_user_details.html", data=data, trans=data['trans'],
                                   _external=True)
        mail.send(msg)
...
def send_user_mail_gym_booked(data):
    with app.app_context():
        ...
        msg.html = render_template("email_user_gym_booked.html", time_period=data['time_period'], s_code=data['s_code'],
                                   total_cost=data['total_cost'], data=data, _external=True)
        mail.send(msg)
...
def send_gym_owner_mail_user_booked_gym(data):
    with app.app_context():
        ...
        msg.html = render_template("email_gym_owner_mail_user_booked_gym.html", time_period=data['time_period'],
                                   data=data, s_code=data['s_code'], total_cost=data['total_cost'], _external=True)
        mail.send(msg)
    ...
    if request.method == 'POST':
        ...
        if order:
            ...
            for i in range(len(update)):
                ...
                response = json.dumps({"status": "success", "updates": updates, "itemsJson": order.items_json},
                                      default=str)
            return response
        else:
